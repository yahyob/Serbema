...
function Register() {
  ...
  const onFinish = (values) => {
    ...
    RegisterApi.create(values)
      .then((data) => {
        setUserData(data);
        message.success("Successfully implemented", [2]);
        message.success("A confirmation code has been sent to your email", [2]);
        setLoading(false);
      })
      .catch((error) => {
        if (error.response?.data?.email?.length !== 0) {
          message.warning("A user with this email already exists", [2]);
          setLoading(false);
          navigate("/login");
          return 1;
        ...
        console.log(error);
        message.error("The process failed", [2]);
        setLoading(false);
      });
  };
